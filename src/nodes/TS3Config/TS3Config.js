"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var ts3_nodejs_library_1 = require("ts3-nodejs-library");
var nodeInit = function (RED) {
    function Ts3Config(config) {
        var _this = this;
        RED.nodes.createNode(this, config);
        var host = config.host;
        var nickname = config.nickname;
        this.name = config.name;
        var client = null;
        var connectPromise = null;
        var shouldDissconect = false;
        this.getConnection = function () { return __awaiter(_this, void 0, void 0, function () {
            var err_1;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!client) return [3 /*break*/, 1];
                        return [2 /*return*/, client];
                    case 1:
                        if (!connectPromise) return [3 /*break*/, 2];
                        return [2 /*return*/, connectPromise];
                    case 2:
                        _a.trys.push([2, 4, , 5]);
                        connectPromise = ts3_nodejs_library_1.TeamSpeak.connect({
                            host: host,
                            protocol: ts3_nodejs_library_1.QueryProtocol.RAW,
                            queryport: 10011,
                            serverport: 9987,
                            username: this.credentials.username,
                            password: this.credentials.password,
                            nickname: nickname
                        });
                        return [4 /*yield*/, connectPromise];
                    case 3:
                        client = _a.sent();
                        if (client == null) {
                            throw new Error("Failed to connect to Teamspeak");
                        }
                        client.on("close", function () { return __awaiter(_this, void 0, void 0, function () {
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        if (!!shouldDissconect) return [3 /*break*/, 4];
                                        if (!(client != null)) return [3 /*break*/, 2];
                                        return [4 /*yield*/, client.reconnect(-1, 1000)];
                                    case 1:
                                        _a.sent();
                                        return [3 /*break*/, 3];
                                    case 2:
                                        this.emit("error");
                                        _a.label = 3;
                                    case 3: return [3 /*break*/, 5];
                                    case 4:
                                        this.emit("disconnect");
                                        _a.label = 5;
                                    case 5: return [2 /*return*/];
                                }
                            });
                        }); });
                        this.emit("connected");
                        return [2 /*return*/, client];
                    case 4:
                        err_1 = _a.sent();
                        this.emit("error");
                        throw err_1;
                    case 5: return [2 /*return*/];
                }
            });
        }); };
        this.on("close", function () { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        shouldDissconect = true;
                        return [4 /*yield*/, (client === null || client === void 0 ? void 0 : client.quit())];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        }); });
    }
    RED.nodes.registerType("TS3Config", Ts3Config, {
        credentials: {
            username: { type: "text" },
            password: { type: "password" }
        }
    });
};
module.exports = nodeInit;
